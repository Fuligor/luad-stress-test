...
from torchvision.models import swin_b  # type: ignore
...
class SwinEncoder(BaseModel):
    def __init__(self, freeze_: bool = False, pretrained: bool = False):
        super().__init__(freeze_)

        self.model = swin_b(weights="DEFAULT" if pretrained else None)
        self._num_features = self.model.head.in_features
