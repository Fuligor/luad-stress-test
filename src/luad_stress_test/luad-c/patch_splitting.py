...
from stress_test_preprocessing.preprocessing.polygon_extraction import extract_coords_from_xml, get_bbox
...
def save_patch(
    ...
    if mask_pattern_pct_area < mask_area_criterion:
        ...
        if save_mask:
            mask_path = patch_dir / "bad_mask" / f"{patch_name}_x{left}_y{top}.{img_ext}"
...
def save_patches_from_dir(
    ...
    for xml_file in xml_dir.iterdir():
        ...
        region_desc, patch_count, oo_range_count = save_patches_from_file(
            slide_file, dest_dir, slide_level, verticies, patch_size, patch_pattern_area_criterion, target_mpp, 
    ...
    with open(dest_dir / "description.json", "w") as f:
...
if __name__ == "__main__":
    ...
    save_patches_from_dir(
        IMG_DIR,
        ADN_DIR / "TCGA-LUAD-patterns",
        PROCESSED_DIR / f"TCGA-LUAD_pattern_patches_{img_size}x{img_size}_v{version}",
