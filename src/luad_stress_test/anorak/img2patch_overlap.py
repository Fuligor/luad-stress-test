...
def get_label_file(data_file, label_path):  # get mask with corresponding name
    ...
    label_file_name = (
        data_file_name[:-4] + ".mat"
    )  # remove .jpg/.png, combined with '.mat'
...
def write_to_patch(data_files, label_path, save_path):
    for file_n in range(0, len(data_files)):
        ...
        num_patch = extract_patches_img_label(
            data,
            labels,
            save_path,
            file_base_name,
...
def extract_patches_img_label(
    ...
    if max(image.shape[0], image.shape[1]) < 0:
        patch_img = image
        cv2.imwrite(
            os.path.join(save_path, "image", file_base_name + ".png"), patch_img
        )
        mask = label
        ...
        image = np.lib.pad(
            image, ((0, pad_h), (0, pad_w), (0, 0)), "constant", constant_values=0
        )
        label = np.lib.pad(
            label, ((0, pad_h), (0, pad_w)), "constant", constant_values=0
        )
        ...
        img_patches = np.zeros(
            (num_patches_img, img_patch_h, img_patch_w, image.shape[2]),
            dtype=image.dtype,
        )
        label_patches = np.zeros(
            (num_patches_img, label_patch_h, label_patch_w), dtype=image.dtype
        )
            ...
            for w in range(int(math.ceil((img_w - img_patch_w) / stride_w + 1))):
                ...
                cv2.imwrite(
                    os.path.join(
                        save_path,
                        "image",
                        file_base_name + "_" + str(iter_tot) + ".png",
                    ),
                    patch_img,
                )
