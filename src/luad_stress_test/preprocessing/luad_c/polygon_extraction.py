...
def extract_coords_from_xml(
    ...
    annot_objects: dict[str, list[list[tuple[int, ...]]]] = {}
...
def save_polygons_from_file(
    ...
    for cls_name, cls_polygons in verticies.items():
        ...
        for i, polygon in enumerate(cls_polygons):
            ...
            masked_image = cv2.bitwise_and(  # pylint: disable=no-member
                patch_np, patch_np, mask=mask_np
            )
            masked_image = cv2.resize(  # pylint: disable=no-member
                masked_image,
                tgt_size,
...
def save_polygons_from_dir(
    ...
    for file in xml_dir.iterdir():
        ...
        extracted_items_desc = save_polygons_from_file(
            PathManager.luad_c_images() / f"{slide_name}.svs",
            PathManager.luad_c_annotations() / f"{slide_name}.xml",
            dest_dir=dest_dir,
    ...
    if save_description_json:
        with open(dest_dir / "descrption.json", "w", encoding="utf-8") as out_file:
